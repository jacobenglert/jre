...
vPast <- read_csv('countypres_2000-2016.csv') %>%
  mutate(party = case_when(party == 'democrat' ~ 'DEM',
                           party == 'republican' ~ 'REP',
                           !(party %in% c('democrat','republican')) ~ 'Other'),
         candidate = ifelse(party %in% c('DEM','REP'), candidate, 'Other')) %>%
  summarise(votes = sum(candidatevotes),
            .by = c(year, state, county, candidate, party))

v2020 <- read_csv("president_county_candidate.csv") %>%
  mutate(county = gsub(pattern = ' County', x = county, replacement = '', fixed = TRUE),
...
write_csv(vComp, 'Data/votes.csv')
```
...
```
```{r label = benford, eval = TRUE, echo = FALSE, out.width="90%", fig.cap='Distribution of Vote Totals', fig.align='center'}
knitr::include_graphics('Figures/benford_compare.png')
```
...
```
```{r, eval = TRUE, echo = FALSE, out.width="90%", fig.cap='State Map', fig.align='center'}
knitr::include_graphics('Figures/state_map.png')
```
...
```{r, eval = TRUE, echo = FALSE, out.width="90%", fig.cap='Country Map', fig.align='center'}
knitr::include_graphics('Figures/country_map.png')
```
